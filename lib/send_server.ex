defmodule SendServer do
  ...
  def handle_info(:retry, state) do
    {failed, done} =
      Enum.split_with(state.emails, fn item -> item.status == "failed" && item.retries < state.max_retires end)

      retried =
        Enum.map(failed, fn item -> IO.puts("Retrying email #{item.email}...")
        new_status =
          ...
          %{email: item.email, status: new_status, retries: item.retries + 1} end)
  ...
  def terminate(reason, _state) do
    IO.puts("Terminating with reaso #{reason}")
  end
end
